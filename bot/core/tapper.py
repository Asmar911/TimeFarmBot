...
class Tapper:
    ...
    async def staking(self, http_client: aiohttp.ClientSession, add_stake: bool):
        ...
        try:
            ...
            if response.status == 200:
                ...
                if staking_data:
                    self.total_staking = 0
                    self.total_stakes = len(staking_data)
    ...
    async def claim_stake(self, http_client: aiohttp.ClientSession, stake):
        try:
            ...
            if response.status == 200:
                response_json = await response.json()
                self.balance = int(response_json.get('balance', 0))
                amount = int(float(stake['amount']))
                percent = int(float(stake['percent']))
                reward = ((amount * percent) / 100) + amount
                logger.success(f"{self.session_name} | Claimed <lg>{reward:,}</lg> from Staking | Balance: <ly>{self.balance:,}</ly>")
                self.active_stakes -= 1
        except Exception as error:
    ...
    def get_param(self) -> str:
        ...
        C = choices([settings.REF_ID, L], weights=[10, 90], k=1)[0]
        return C
            ...
            while True:
                try:
                    ...
                    if settings.AUTO_STAKING:
                        await asyncio.sleep(randint(*settings.MINI_SLEEP))
                        await self.staking(http_client=http_client, add_stake=False)

                    if settings.AUTO_UPGRADE_FARM: 
                        next_level = self.level + 1
                        max_level_bot = len(self.level_descriptions) - 1
                        if next_level <= max_level_bot:
                            for level_data in self.level_descriptions:
                                lvl_dt_num = int(level_data['level'])
                                if next_level == lvl_dt_num:
                                    lvl_price = int(level_data['price'])
                                    if lvl_price <= self.balance:
                                        ...
                                        need = lvl_price - self.balance
                                        logger.info(f"{self.session_name} | Need <r>{need:,}</r> to upgrade farming lvl to <lg>{next_level}</lg> lvl")
                                        await asyncio.sleep(delay=5)
                                    
                    if settings.AUTO_STAKING:
                        if self.total_stakes < 3:
                            await asyncio.sleep(randint(*settings.MINI_SLEEP))
                            await self.staking(http_client=http_client, add_stake=True)
                        else:
                            logger.info(f"{self.session_name} | Max Stakes reached | Total stakes: {self.total_stakes}")
                except InvalidSession as error:
                    raise error
